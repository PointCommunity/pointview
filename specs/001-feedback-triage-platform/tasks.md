...
- [x] **T026** [P] [US2] Implement the provider-neutral decision interface, strict schema validation, hosted web-research/source capture, usage accounting, timeouts, and fixture transport — **Acceptance**: model-visible context receives no credentials or mutation tools, output parses only through the versioned schema, and normal tests make no live call; **Verify**: transport/contract/timeout tests against `triage-decision.schema.json` plus idempotent PostgreSQL source capture; **Depends**: T007; **Files**: `src/server/triage/model/**`, `src/server/research/web-captures.ts`, provider contract tests, `tests/integration/feedback-intake.test.ts`.
...
## Phase 6A — Provider connections and guided Owner settings

**Goal**: An Owner can connect Ollama Cloud and/or OpenAI Codex entirely through an understandable web experience, select one discovered model, and run triage without an OpenAI Platform API key.

- [x] **T056** [US4] Add restart-safe provider-connection persistence and AES-256-GCM credential envelopes — **Acceptance**: one row per provider, associated-data binding, key/version validation, no plaintext persistence/readback, safe disconnect, and migration rollback; **Verify**: unit encryption tests, PostgreSQL migration/upsert tests, tamper and cross-provider replay tests; **Files**: `migrations/0002_provider_connections.sql`, `migrations/0002_down.sql`, `src/server/providers/credentials.ts`, `src/server/providers/repository.ts`, `src/server/db/migrations.ts`, tests.
- [x] **T057** [US4] Implement Ollama Cloud connect, authenticated `/api/tags` discovery, `/api/show` capability discovery, bounded chat adapter, and deterministic JSON repair/validation — **Acceptance**: invalid keys and provider errors are safely classified, only authenticated catalog models can be selected, screenshot bytes reach only vision-capable models, Cloud structured-output absence is handled by PointView validation, and credentials never enter provider-visible content/logs; **Verify**: transport, capability, timeout, malformed JSON, schema failure, repair ceiling, and credential-redaction tests; **Depends**: T056; **Files**: `src/server/providers/ollama.ts`, `src/server/triage/model/ollama.ts`, provider routes, tests.
- [x] **T058** [US4] Implement pinned Codex app-server device-code login, managed credential persistence, `model/list` discovery, and isolated non-interactive decision execution — **Acceptance**: OAuth tokens never reach the browser, incomplete sessions fail closed, model runs use an empty read-only ephemeral workspace with no inherited application secrets/MCP/rules, and refreshed credentials are re-encrypted; **Verify**: JSON-RPC fixture, session expiry/restart, catalog, sandbox/config argument, schema output, cleanup, and redaction tests; **Depends**: T056; **Files**: `src/server/providers/codex/**`, `src/server/triage/model/codex.ts`, provider routes, Docker/package files, tests.
- [x] **T059** [US4] Replace raw settings with accessible provider cards, device-login steps, model picker, pause, and retention controls — **Acceptance**: no raw secret reference/provider identifier/retrieval JSON/prompt/schema/token/timeout field appears; both providers show loading/empty/error/connected states; available models are grouped and image support is explained; 320/768/1280 layouts and keyboard/axe checks pass; **Verify**: component/contract and Playwright tests; **Depends**: T057–T058; **Files**: `src/components/admin/settings-admin.tsx`, `src/app/admin/settings/page.tsx`, provider/settings routes, CSS, E2E tests.
- [x] **T060** [US4] Route daily triage through the selected connected provider and fail readiness closed on stale configuration — **Acceptance**: no OpenAI Platform SDK/key remains, selected catalog membership is revalidated, no model/GitHub call occurs when disconnected/stale, usage and provider identity stay auditable, and both adapters satisfy the same decision contract; **Verify**: config, readiness, CLI, adapter, integration, security, and full queue fixtures; **Depends**: T057–T059; **Files**: `src/cli/triage-once.ts`, `src/server/config/**`, `src/server/operations/readiness.ts`, tests/docs/contracts.
