...
export async function appendAuditEvent(sql: postgres.Sql, input: Omit<AuditEventInput, "previousEventHash">) {
  return sql.begin(async (tx) => {
    const [previous] = await tx<{ eventHash: string }[]>`
      select event_hash as "eventHash" from audit_events order by event_at desc, id desc limit 1 for update
    `;
