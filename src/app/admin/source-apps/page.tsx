...
export default async function SourceAppsPage() {
  ...
  return <AppShell><section className="page-heading compact"><p className="eyebrow">Owner controls</p><h1>Source integrations.</h1><p className="lede">Each source is pinned to one repository, one private Project, exact origins, and rotating public launch keys.</p></section><SourceAdmin initialSources={sources.map(presentSourceApp)} history={history.map((event) => ({ ...event, eventAt: event.eventAt.toISOString() }))} csrfToken={cookieStore.get(csrfCookie.name)?.value ?? ""} /></AppShell>;
}
